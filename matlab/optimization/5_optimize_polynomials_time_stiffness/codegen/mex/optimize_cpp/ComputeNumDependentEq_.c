...
int32_T ComputeNumDependentEq_(f_struct_T *qrmanager, const emxArray_real_T
  *beqf, int32_T mConstr, int32_T nVar)
{
  ...
  if (numDependent > 0) {
    ...
    for (idx = 0; idx < y; idx++) {
      ...
      b_xcopy((mConstr - idx) - 1, qrmanager->QR, iQR0, qrmanager->Q, iQR0);
    }

    xorgqr(mConstr, mConstr, qrmanager->minRowCol, qrmanager->Q, qrmanager->ldq,
           qrmanager->tau);
    idx = 0;
    exitg1 = false;
    while ((!exitg1) && (idx <= numDependent - 1)) {
      x = xdot(mConstr, qrmanager->Q, qrmanager->ldq * ((mConstr - idx) - 1) + 1,
               beqf);
