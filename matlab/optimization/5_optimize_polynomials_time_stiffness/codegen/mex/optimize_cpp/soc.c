...
boolean_T soc(const emxArray_real_T *Hessian, const emxArray_real_T *grad,
              ...
              *WorkingSet, f_struct_T *QRManager, h_struct_T *CholManager,
              i_struct_T *QPObjective, const b_struct_T *qpoptions)
{
  ...
  c_xcopy(WorkingSet->mConstrMax, TrialState->lambda, TrialState->lambda_old);
  ...
  oldDirIdx = xnrm2(nVar, TrialState->socDirection);
  lenQPNormal = xnrm2(nVar, TrialState->xstar);
  ...
  if (!success) {
    ...
    for (idx_IneqLocal_tmp = idx_Aineq; idx_IneqLocal_tmp <= idx_lower;
      ...
      WorkingSet->isActiveConstr->data[(WorkingSet->isActiveIdx[WorkingSet->
        Wid->data[idx_IneqLocal_tmp - 1] - 1] + WorkingSet->Wlocalidx->
        data[idx_IneqLocal_tmp - 1]) - 2] = false;
    }
    ...
    for (idx = 0; idx < nWIneq_old; idx++) {
      ...
      WorkingSet->bwset->data[WorkingSet->nActiveConstr - 1] = WorkingSet->
        bineq->data[TrialState->workingset_old->data[idx] - 1];
    }
    ...
    for (idx = 0; idx < nWUpper_old; idx++) {
      addBoundToActiveSetMatrix_(WorkingSet, 5, TrialState->workingset_old->
        data[(idx + mIneq) + idx_upper]);
    }

    c_xcopy(mConstrMax, TrialState->lambda_old, TrialState->lambda);
  } else {
