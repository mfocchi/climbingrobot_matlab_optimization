...
boolean_T step(int32_T *STEP_TYPE, emxArray_real_T *Hessian, const
               ...
               *TrialState, k_struct_T *MeritFunction, c_struct_T *memspace,
               j_struct_T *WorkingSet, f_struct_T *QRManager, h_struct_T
               *CholManager, i_struct_T *QPObjective, b_struct_T *qpoptions)
{
  ...
  if (*STEP_TYPE != 3) {
    ...
    c_xcopy(WorkingSet->nVar, TrialState->xstar, TrialState->searchDir);
  }
    ...
    switch (*STEP_TYPE) {
     case 1:
      ...
      if ((TrialState->state <= 0) && (TrialState->state != -6)) {
        *STEP_TYPE = 2;
        ...
        c_xcopy(nVar, TrialState->xstar, TrialState->delta_x);
        guard1 = true;
      }
      break;
     ...
     case 2:
      ...
      for (idxStartIneq = 0; idxStartIneq < idxEndIneq; idxStartIneq++) {
        nrmGradInf = WorkingSet->lb->data[WorkingSet->indexLB->data[idxStartIneq]
          - 1];
        if (-r->data[WorkingSet->indexLB->data[idxStartIneq] - 1] > nrmGradInf)
        {
          if (muDoubleScalarIsInf(ub->data[WorkingSet->indexLB->
            ...
            r->data[WorkingSet->indexLB->data[idxStartIneq] - 1] =
              (WorkingSet->ub->data[WorkingSet->indexLB->data[idxStartIneq] - 1]
               - nrmGradInf) / 2.0;
          }
        }
      }
      ...
      for (idxStartIneq = 0; idxStartIneq < mUB; idxStartIneq++) {
        nrmGradInf = WorkingSet->ub->data[WorkingSet->indexUB->data[idxStartIneq]
          - 1];
        if (r->data[WorkingSet->indexUB->data[idxStartIneq] - 1] > nrmGradInf) {
          if (muDoubleScalarIsInf(lb->data[WorkingSet->indexUB->
            ...
            r->data[WorkingSet->indexUB->data[idxStartIneq] - 1] = (nrmGradInf -
              WorkingSet->lb->data[WorkingSet->indexUB->data[idxStartIneq] - 1])
              / 2.0;
          }
        }
      }
