...
void iterate(const emxArray_real_T *H, const emxArray_real_T *f, d_struct_T
             *solution, c_struct_T *memspace, j_struct_T *workingset, f_struct_T
          ...
          if ((normDelta < options_StepTolerance) || (workingset->nActiveConstr >=
            ...
            if (updateFval) {
              ...
              if (objective->objtype == 5) {
                normDelta = xnrm2(objective->nvar, solution->searchDir);
            ...
            xaxpy(nVar, c, solution->searchDir, solution->xstar);
            computeGrad_StoreHx(objective, H, f, solution->xstar);
            updateFval = true;
            guard1 = true;
          }
      ...
      if (guard1) {
        ...
        if (solution->iterations - solution->iterations / 50 * 50 == 0) {
          solution->maxConstr = maxConstraintViolation(workingset,
            solution->xstar);
          if (solution->maxConstr > 0.0001) {
            c_xcopy(objective->nvar, solution->xstar, solution->searchDir);
