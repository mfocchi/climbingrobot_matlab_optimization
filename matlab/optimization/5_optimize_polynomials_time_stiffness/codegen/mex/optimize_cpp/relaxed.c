...
void relaxed(const emxArray_real_T *Hessian, const emxArray_real_T *grad,
             ...
             *memspace, j_struct_T *WorkingSet, f_struct_T *QRManager,
             ...
             *qpoptions)
{
  ...
  if (TrialState->sqpIterations <= 1) {
    ...
    idx_max = ixamax(WorkingSet->mConstr, TrialState->lambdasqp);
  ...
  for (idx = 0; idx < b_mIneq; idx++) {
    ...
    if (memspace->workspace_double->data[idx] <= 0.0001) {
  ...
  if (TrialState->state != -6) {
    ...
    qpfvalLinearExcess = xasum(idx_max, TrialState->xstar, nVarOrig);
    qpfvalQuadExcess = c_xdot(idx_max, TrialState->xstar, nVarOrig,
      TrialState->xstar, nVarOrig);
    updatePenaltyParam(MeritFunction, TrialState->sqpFval, TrialState->cIneq,
                       mIneq, TrialState->sqpIterations, (TrialState->fstar -
      rho * qpfvalLinearExcess) - beta / 2.0 * qpfvalQuadExcess,
