function [target, l_vec, theta_r] = compute_params_from_target(int_target, N)
...
%     target = [x,y,z];
    ...
    for i=1:N
        angle = sum( theta_vec(1:i)) - sum(theta_r(1:i));
        int_target(:,i) = [0;  l_vec(i)*sin(angle); -l_vec(i)*cos(angle)];
    end
