function [p, E,  path_length, initial_error, final_error] = eval_solution_mj(x,  dt, p0, pf)
...
% velocity
pd = [cos(theta).*cos(phi).*thetad.*l - sin(phi).*sin(theta).*phid.*l + sin(theta).*cos(phi).*ld;
    cos(theta).*sin(phi).*thetad .*l + cos(phi).*sin(theta).*phid.*l + sin(theta).*sin(phi).*ld;
    sin(theta).*thetad.*l - cos(theta).*ld];
...
% E.Etot =  ((m*l.^2/2.*(thetad.^2 + sin(theta).^2 .*phid.^2))) - m*g*l.*cos(theta)+ K*(l-l_uncompressed).^2/2;
