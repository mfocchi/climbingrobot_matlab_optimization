...
"conservation of energy". As far as the forces applied to the system
...
\begin{equation}
\begin{aligned}
&T = \frac{m }{2} l^2 \left(\dot{\theta}^2 + s_\theta^2 \dot{\phi}^2 \right) + \frac{m }{2} \dot{l}^2 \\
...
Encluding the instant $t = 0$ from energy conservation allows us to discharge the effect of the non conservative term $\mathbf{F}_u$,
...
$F_{u,t}$ and $F_{u,n}$) and of the elastic force parameters $k$ and
$l_0$).
...
it is possible by requiring a constant energy at the two boundaries. We get one equation and one unknowns. The problem is that
...
\item $\min T(t_f)$ if we aim to minime the residual kinetic energy which will have to be dissipated when the robot lands,
...
Once the optimisation problem outlined above has been solved, we find a reference trajectory (given by the cubic expressions $p_a(t)$, $p_b(t)$, $p_c(t)$).
