...
The system actuators are given by the two forces $\mathbf{F}_r$ and $\mathbf{F}_n$.
...
\]
As regards $\mathbf{F}_n$, since it acts in a very small time reaching high peaks, we model it through a diract impulse: $\mathbf{F}_n   (t) = \mathbf{F}_n \delta(t)$, where
$\mathbf{F}_n$ is a vector with the two components $F_{u,t}$ and $F_{u,n}$.
...
slack variable $\Delta$ and have a constraint $\left|\vect{x}(t_f) - \vect{X}_f\right\| \leq \Delta$, where the slack $\Delta$ can be either a maximum tolerance set by the user or became part of the cost function.
