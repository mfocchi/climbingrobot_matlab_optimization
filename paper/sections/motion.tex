...
\begin{equation}\label{eq:OCP}
  \begin{aligned}
    &\min_{\vect{u}(t)} \,M(\vect{x}(t_0),\vect{x}(t_f),t_0,t_f)+\int_{t_0}^{t_f}   L\left(\vect{x}(t),\vect{u}(t),t\right) dt\\
    &\text{subject to:}\\
...
(assumption 3, the rewinder can only pull the wire), $F_r$ can only be used to retract the robot or to slow down its descent. What is more, the force is limited in with $F_r^{\text{max}}>0$, due to the actuators constraints. Therefore, the
...
A natural approximation is to use a Gaussian function, that is $\delta(t-t_0) \approx \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\frac{(t-t_0)^2}{2 \sigma^2}}$. The duration of the impulse
...
Therefore, we optimise
\begin{align*}
&  w_t \int_{t_0}^{t_f}dt + w_E T(t_f)  \\
                                        &= w_t (t_f - t_0) + w_E \frac{m}{2} x_3(t_f)^2 \left(x_4(t_f)^2 + s^2_{x_1(t_f)}+ x_5(t_f)^2 \right) \\
