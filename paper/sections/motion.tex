...
\begin{equation}\label{eq:OCP}
  ...
  \end{aligned}
\end{equation}
\ADP{If I am not wrong our dynamics is time-invariant}
...
the state variables inverting~\eqref{eq1}. \ADP{Agreed, but that's not necessary. You can express the constraints directly in Cartesian space using \eqref{eq1}, without inverting it.} For instance, when time equals zero \ADP{Why does time need to be zero for these equations to be valid? Eq.~\eqref{eq1} is time invariant} (initial conditions), we have:
\begin{equation}
  \begin{aligned}
...
A natural approximation is a Gaussian function, that is $\delta(t-t_0) \approx \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\frac{(t-t_0)^2}{2 \sigma^2}}$. The duration $T_{th}$ of the impulse 
  ...
  determines a remarkable height \ADP{Not clear what is meant here with "obviously determine a remarkable height". What height are you talking about? You mean the intensity of the impulse must be large because the duration is small?} for the impulse. The maximum norm of $\mathbf{F}_n$ \ADP{Should this be $F_u$?} is upper-bounded by the actuation limits:
  \[
...
\end{align*} 
where $\mu$ is a constant depending on the nature of the terrain. For our simulation scenarios below, we have chosen $\mu = 0.8$ and \ADP{$F_u$?} $F_n^{\text{max}}=1000$ N. \ADP{These are both unrealistic values, but I guess you knew it already! :)}
...
\end{align*}
\ADP{Shouldn't $w_E$ multiply also the last term?}
%%%
...
\noindent
\textbf{Initial guess.}
To speedup convergence we can initialize the optimization with a reasonable initial guess.
Because time $t_f$ is an optimization parameter, we compute the time constant for the system linearized around the initial state. \ADP{Ok, but what about the other variables? You only provide the initial guess for the time. It's not clear from the text.}
