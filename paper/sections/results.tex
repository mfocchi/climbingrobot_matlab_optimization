...
%
\begin{equation}
\boldsymbol{\tau}_{\text{leg}}^d= \mat{\boldsymbol{\tau}_{HP} \\ \boldsymbol{\tau}_{HR} \\ \vect{f}_{K}} =  \vect{h}_{\text{leg}} -J_{\text{leg}}^T \underbrace{\left(R_e^\mathcal{W}  \mat{F_{u,n} \\ F_{u,t} \\ 0} \right) 	}_{\vect{F}_c}
\end{equation}
%
...
applying $\boldsymbol{\tau}_{\text{leg}}$ for the thrust duration $T_{th} = 0.025 s$, while the PD gains are 
...
full-model simulation the norm of the final error is around 0.75 $m$ for a 12 $m$ jump (0.6\%). This is due to a number of reasons: 
