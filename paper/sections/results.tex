...
(see Fig. \ref{fig:targets}), starting from a point on the wall $p_0 = \mat{0,0,-8}$.
To show that the approach is valid also for a jump from a non vertical (i.e. slanted ) surface, 
we started from a location $p_0 $ that is already on the pillar itself which has an inclination around 80 $deg$.
...
The results are the reported in Table \ref{tab:sim_different_targets} and also 
shown in the accompanying video\footnote{ \href{https://www.dropbox.com/s/b0fuyiewyp3jkg7/icra23climb.mp4}
...
\begin{table}[!tbp]
	...
	\resizebox{\columnwidth}{!}{
		\begin{tabular}{c l  l c c c c  c } \hline\hline
			\textbf{Exp.}  & \textbf{$p_0$ [m]}  & \textbf{$p_f$ [m]}           & \textbf{$T_f$ [s]} &  \textbf{$F_{un}$ [N]} & \textbf{$F_{ut}$ [N]} 
			...
			1   &$ \mat{0 & 0 &-8}$            & $\mat{2.60 & 2.73 & -20.46} $    &      2.16      &         780             &      624           &    408    \\ % exp2
			2   &$ \mat{0 & 0 &-8}$            & $\mat{ 2.50 &  0.05 &   -20.46}$ &      1.57        &         241             &      192           &    6.5               \\ % exp3
			3   &$ \mat{0 & 0 &-8}$            & $\mat{0.55 & -0.8 &  -17.76} $   &      1.2         &       746                  &   200              & 11.8               \\ % exp5
			4   &$ \mat{0.63 & 2.35& -7.5}$   & $\mat{1.73 & 3.8 &  -20.46} $     &      1.54       &        504              &     403         &      22    &                 \\ % exp6_slanted		
			\hline\hline 					    					    					    
	\end{tabular}}
...
\begin{figure}
	...
	\caption{\small Simulation. The black star is the anchor point, the green dot is the starting point $p_0$ the red dot the target location $p_f$.
			...
			the friction boundaries ($\mu = 0.7$) in the tangential direction. The  cone represents the rock pillar obstacle to avoid.}
	\label{fig:targets}
\end{figure}

Inspecting the red trajectories in  \ref{fig:validation}, one can see that 
