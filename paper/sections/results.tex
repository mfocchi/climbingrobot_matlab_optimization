...
a rock pillar (see the accompanying video~ \footnote{\footnote{\url{https://www.dropbox.com/s/b0fuyiewyp3jkg7/icra23climb.mp4?dl=0}}}) that is laying  the wall, that we model 
...
%
\begin{equation}
\tau_{leg}= \mat{\tau_{HP} \\ \tau_{HR} \\ f_{K}} =  h_{\text{leg}} -J_{\text{leg}}^T \underbrace{\left(R_b^\mathcal{W}  \mat{F_{un} \\ F_{ut} \\ 0} \right) 	}_F
\end{equation}
%
...
applying $\tau_{leg}$ for a time $T_{th} = 0.05 s$ then it starts the 
